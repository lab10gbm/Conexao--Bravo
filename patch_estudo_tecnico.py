...
def patch_estudo():
    ...
    # Find the block where the rendering of alasStats happens:
    # {alasStats.map(stats => { ... return <motion.div ...> ... </motion.div> })}
    
    old_block_pattern = r"\{alasStats\.map\(stats => \{(.*?)\} \)\}"
    
    match = re.search(old_block_pattern, content, re.DOTALL)
    
    new_block = """{alasStats.map(stats => {
            return (
               <motion.div 
               ...
               >
                  <div className={cn("p-4 border-b flex justify-between items-center", `bg-${getAlaColor(stats.ala as '1'|'2'|'3'|'4')}-50`, `border-${getAlaColor(stats.ala as '1'|'2'|'3'|'4')}-100`)}>
                     <div className="flex items-center gap-2">
                        <div className={cn("w-2 h-2 rounded-full", `bg-${getAlaColor(stats.ala as '1'|'2'|'3'|'4')}-500`)} />
                        <h2 className={cn("text-xs font-black uppercase tracking-widest", `text-${getAlaColor(stats.ala as '1'|'2'|'3'|'4')}-700`)}>
                           {getAlaName(stats.ala as '1'|'2'|'3'|'4')}
                        </h2>
                     </div>
                  </div>
