...
export function OfficerGrdModule({ user, obmContext }: OfficerGrdModuleProps) {
  ...
  const [configTab, setConfigTab] = useState<'oficialDia' | 'sobreaviso' | 'ras'>('oficialDia');
      ...
      if (snapshot.exists()) {
        ...
        if (data.lists && data.lists[monthKey]) {
           ...
           if (Array.isArray(currentMonthLists)) {
               ...
               setSelectedRgsRas(currentMonthLists.ras || []);
           }
        } else {
  ...
  const handleToggleOfficer = async (rg: string) => {
    ...
    } else if (configTab === 'ras') {
        newRas = newRas.includes(rg) ? newRas.filter(r => r !== rg) : [...newRas, rg];
  ...
  const distributionQuotas = useMemo(() => {
    const currentActiveList = configTab === 'oficialDia' ? selectedRgsOficialDia : 
                              configTab === 'sobreaviso' ? selectedRgsSobreaviso : selectedRgsRas;
    ...
    return dist;
  }, [militars, selectedRgsOficialDia, selectedRgsSobreaviso, selectedRgsRas, configTab, monthDaysOnly]);

  const handleGenerateScale = async () => {
    const label = configTab === 'oficialDia' ? 'Oficial de Dia' : configTab === 'sobreaviso' ? 'Sobreaviso 1' : 'RAS';
            ...
            {showConfig && (
              <div className="mb-6 p-4 bg-slate-50 border border-slate-200 rounded-md">
                <div className="flex gap-2 mb-4 border-b border-slate-200 pb-2">
                   ...
                   >Sobreaviso 1</button>
                   <button 
                     onClick={() => setConfigTab('ras')}
                     className={cn("px-3 py-1 text-[11px] font-black uppercase tracking-widest rounded-t-md transition-all", configTab === 'ras' ? "bg-[#406b53] text-white" : "bg-slate-200 text-slate-500 hover:bg-slate-300")}
                   >RAS</button>
                </div>
                <h4 className="text-xs font-bold uppercase text-slate-500 mb-3">Selecione os Oficiais para a Escala ({configTab === 'oficialDia' ? 'Oficial de Dia' : configTab === 'sobreaviso' ? 'Sobreaviso' : 'RAS'}):</h4>
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 max-h-48 overflow-y-auto p-1 custom-scrollbar">
                  {availableOfficers.map(mil => {
                    const isSelected = configTab === 'oficialDia' ? selectedRgsOficialDia.includes(mil.rg) :
                                       configTab === 'sobreaviso' ? selectedRgsSobreaviso.includes(mil.rg) : 
                                       selectedRgsRas.includes(mil.rg);
                    return (
                      <button
                        key={mil.rg}
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-[#406b53] border-b border-slate-400">
                ...
                <th className="px-3 py-4 text-[11px] font-black uppercase tracking-widest text-white border-r border-[#305542]">OFICIAL DE DIA / CMT OP</th>
                <th className="px-3 py-4 text-[11px] font-black uppercase tracking-widest text-white border-r border-[#305542]">SOBREAVISO 1 / GRD 1</th>
                <th className="px-3 py-4 text-[11px] font-black uppercase tracking-widest text-white">RAS</th>
              </tr>
            </thead>
            <tbody>
                ...
                return (
                  ...
                  >
                    ...
                    {['oficialDia', 'sobreaviso', 'ras'].map((field) => {
                      ...
                      return (
                        <td key={field} className="px-2 py-2 border-r border-slate-200 relative w-1/4">
                          <button
                            onClick={() => {
                              ...
                              <div className="max-h-60 overflow-y-auto space-y-1 custom-scrollbar">
                                ...
                                {militars
                                  .filter(m => {
                                    ...
                                    if (isOff) {
                                      const role = (m.officerRole || '').toUpperCase();
                                      if (role.includes('MÉDICO') || role.includes('MEDICO') || role.includes('ADMINISTRATIVO')) isCombatente = false;
                                      const q = (m.quadro || '').toUpperCase();
