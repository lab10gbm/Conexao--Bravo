...
import { parseRank, COLS_OFICIAIS, RANKS_PRACAS, isOfficer, isPraca } from '../lib/rankUtils';
import { Columns, LayoutGrid } from 'lucide-react';

export function EfetivoResumoTables({ militars, onFilterClick }: { militars: UserProfile[], onFilterClick?: (f: { rank?: string, quadro?: string, ala?: string }) => void }) {
  const [viewType, setViewType] = useState<'quadro' | 'ala'>('quadro');

  const handleCellClick = (f: { rank?: string, quadro?: string, ala?: string }) => {
    if (onFilterClick) onFilterClick(f);
  };

  // --- OFICIAIS ---
  const oficiais = militars.filter(m => isOfficer(parseRank(m.rank)));
  const quadrosOficiais = Array.from(new Set(oficiais.map(m => m.quadro?.toUpperCase() || 'S/Q'))).sort();
  ...
  oficiais.forEach(m => {
     const q = m.quadro?.toUpperCase() || 'S/Q';
  ...
  const quadrosPracas = Array.from(new Set(pracas.map(m => m.quadro?.toUpperCase() || 'S/Q'))).sort();
  ...
  // --- ALA DATA (Praças) ---
  const alas = Array.from(new Set(pracas.map(m => m.ala?.toString() || 'S/A'))).sort() || [];
         ...
         <>
           ...
           <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
             <div className="bg-gradient-to-r from-[#8cc63f] to-[#1e3b70] p-3 border-b-4 border-[#1e3b70]">
               <h3 className="text-white font-black text-base sm:text-lg tracking-wider text-center drop-shadow-md">OFICIAIS</h3>
             </div>
             <div className="overflow-x-auto no-scrollbar relative">
                <div className="sm:hidden mb-2 flex items-center gap-1.5 px-2 py-1 bg-slate-50 border-b border-slate-100">
                  ...
                  <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Deslize para ver quadro →</span>
                </div>
                <table className="w-full text-center border-collapse min-w-[500px]">
                   <thead>
                      <tr className="bg-slate-100/50 text-[9px] sm:text-[10px] font-black tracking-widest text-slate-700">
                         <th className="border border-slate-200 p-1.5 sm:p-2 uppercase text-left w-32 sm:w-40">QUADRO</th>
                         {COLS_OFICIAIS.map(r => <th key={r} className="border border-slate-200 p-1.5 sm:p-2 uppercase">{r}</th>)}
                         <th className="border border-slate-200 p-1.5 sm:p-2 uppercase bg-slate-100">TOTAL</th>
                      </tr>
                   </thead>
                      ...
                      {quadrosOficiais.map(q => {
                         ...
                         return (
                           <tr key={q} className="hover:bg-slate-50 transition-colors">
                             <td className="border border-slate-200 p-1.5 sm:p-2 font-bold text-left">{q}</td>
                             {COLS_OFICIAIS.map(r => {
                                ...
                                return (
                                  <td 
                                    ...
                                    onClick={() => val > 0 && handleCellClick({ rank: r, quadro: q })}
                                  >
                                    {val > 0 ? String(val).padStart(2, '0') : ''}
                                  </td>
                                );
                             })}
                             <td className="border border-slate-200 p-1.5 sm:p-2 font-bold bg-slate-50">
                               {rowTotal > 0 ? String(rowTotal).padStart(2, '0') : ''}
                             </td>
                           </tr>
                         );
                      })}
                   ...
                   <tfoot className="bg-slate-50 font-black text-[11px] sm:text-xs text-slate-800">
                      <tr>
                         <td className="border border-slate-200 p-1.5 sm:p-2 text-left">TOTAL</td>
                         {COLS_OFICIAIS.map(r => {
                            ...
                            return (
                              <td 
                                ...
                                onClick={() => ofiTotalByRank[r] > 0 && handleCellClick({ rank: r })}
                              >
                                {ofiTotalByRank[r]}
                              </td>
                            );
                         })}
                         <td className="border border-slate-200 p-1.5 sm:p-2">{ofiGrandTotal}</td>
                      </tr>
                   </tfoot>
                </table>
             </div>
           ...
           <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden mt-6">
             <div className="bg-gradient-to-r from-[#8cc63f] to-[#1e3b70] p-3 border-b-4 border-[#1e3b70]">
               <h3 className="text-white font-black text-base sm:text-lg tracking-wider text-center drop-shadow-md">PRAÇAS</h3>
             </div>
             <div className="overflow-x-auto no-scrollbar relative">
                <div className="sm:hidden mb-2 flex items-center gap-1.5 px-2 py-1 bg-slate-50 border-b border-slate-100">
                  <div className="w-1.5 h-1.5 bg-blue-500 rounded-full animate-ping" />
                ...
                <table className="w-full text-center border-collapse min-w-[500px]">
                   <thead>
                      <tr className="bg-slate-100/50 text-[9px] sm:text-[10px] font-black tracking-widest text-slate-700">
                         <th className="border border-slate-200 p-1.5 sm:p-2 uppercase text-left w-32 sm:w-40">QBMP</th>
                         {quadrosPracas.map(q => <th key={q} className="border border-slate-200 p-1.5 sm:p-2 uppercase">{q}</th>)}
                         <th className="border border-slate-200 p-1.5 sm:p-2 uppercase bg-slate-100">TOTAL</th>
                      </tr>
                   </thead>
                   <tbody className="text-[11px] sm:text-xs font-medium text-slate-700">
                      {RANKS_PRACAS.map(r => {
                         ...
                         return (
                           <tr key={r} className="hover:bg-slate-50 transition-colors">
                             <td className="border border-slate-200 p-1.5 sm:p-2 font-bold text-left bg-slate-50">{r}</td>
                             {quadrosPracas.map(q => {
                         ...
                         {quadrosPracas.map(q => {
                            ...
                            return (
                              <td 
                                ...
                                onClick={() => pracasTotalByQuadro[q] > 0 && handleCellClick({ quadro: q })}
                              >
                                {pracasTotalByQuadro[q] > 0 ? String(pracasTotalByQuadro[q]).padStart(2, '0') : 0}
                              </td>
                            );
                         })}
                         <td className="border border-slate-200 p-1.5 sm:p-2">{pracasGrandTotal}</td>
                      </tr>
                   </tfoot>
                </table>
             </div>
         ...
         <>
           ...
           <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden mb-6">
             <div className="bg-gradient-to-r from-[#1e3b70] to-[#8cc63f] p-3 border-b-4 border-[#8cc63f]">
               <h3 className="text-white font-black text-base sm:text-lg tracking-wider text-center drop-shadow-md">PRAÇAS (POSTO vs ALA)</h3>
             </div>
             <div className="overflow-x-auto no-scrollbar relative">
                <div className="sm:hidden mb-2 flex items-center gap-1.5 px-2 py-1 bg-slate-50 border-b border-slate-100">
                  <div className="w-1.5 h-1.5 bg-blue-500 rounded-full animate-ping" />
                  <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Deslize para ver ala →</span>
                </div>
                <table className="w-full text-center border-collapse min-w-[500px]">
                   <thead>
                      <tr className="bg-slate-100/50 text-[9px] sm:text-[10px] font-black tracking-widest text-slate-700">
                         <th className="border border-slate-200 p-1.5 sm:p-2 uppercase text-left w-32 sm:w-40">QBMP</th>
                         {alas.map(a => <th key={a} className="border border-slate-200 p-1.5 sm:p-2 uppercase">{a}</th>)}
                         <th className="border border-slate-200 p-1.5 sm:p-2 uppercase bg-slate-100">TOTAL</th>
                      </tr>
                   </thead>
                   <tbody className="text-[11px] sm:text-xs font-medium text-slate-700">
                             ...
                             {alas.map(a => {
                                ...
                                return (
                                  <td 
                                    key={a} 
                                    className={`border border-slate-200 p-1.5 sm:p-2 ${val > 0 ? 'text-indigo-600 font-bold cursor-pointer hover:bg-indigo-50' : ''}`}
                                    onClick={() => val > 0 && handleCellClick({ rank: r, ala: a })}
                                  >
                                    {val > 0 ? String(val).padStart(2, '0') : ''}
                                  </td>
                                );
                         ...
                         {alas.map(a => {
                            return (
                              <td 
                                ...
                                onClick={() => pracasAlaTotal[a] > 0 && handleCellClick({ ala: a })}
                              >
                                {pracasAlaTotal[a] > 0 ? String(pracasAlaTotal[a]).padStart(2, '0') : 0}
                              </td>
                            );
                         })}
                         <td className="border border-slate-200 p-1.5 sm:p-2">{Object.values(pracasAlaTotal).reduce((acc, curr) => acc + curr, 0)}</td>
                      </tr>
                   </tfoot>
                </table>
             </div>
           ...
           <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
             <div className="bg-gradient-to-r from-[#1e3b70] to-[#8cc63f] p-3 border-b-4 border-[#8cc63f]">
               <h3 className="text-white font-black text-base sm:text-lg tracking-wider text-center drop-shadow-md">PRAÇAS (QUADRO vs ALA)</h3>
             </div>
             <div className="overflow-x-auto no-scrollbar relative">
                <div className="sm:hidden mb-2 flex items-center gap-1.5 px-2 py-1 bg-slate-50 border-b border-slate-100">
                  <div className="w-1.5 h-1.5 bg-blue-500 rounded-full animate-ping" />
                ...
                <table className="w-full text-center border-collapse min-w-[500px]">
                   ...
                   <tbody className="text-[11px] sm:text-xs font-medium text-slate-700">
                      {quadrosPracas.map(q => {
                         ...
                         return (
                           <tr key={q} className="hover:bg-slate-50 transition-colors">
                             <td className="border border-slate-200 p-1.5 sm:p-2 font-bold text-left bg-slate-50">{q}</td>
                             {alas.map(a => {
                                ...
                                return (
                                  <td 
                                    key={a} 
                                    className={`border border-slate-200 p-1.5 sm:p-2 ${val > 0 ? 'text-indigo-600 font-bold cursor-pointer hover:bg-indigo-50' : ''}`}
                                    onClick={() => val > 0 && handleCellClick({ quadro: q, ala: a })}
                                  >
                                    {val > 0 ? String(val).padStart(2, '0') : ''}
                                  </td>
                                );
