...
// src/server/routes/sync.ts
var syncRouter = import_express2.default.Router();
var apiKeyMiddleware = (req, res, next) => {
...
var bulkSyncHandler = async (req, res) => {
  ...
  try {
    ...
    const serverTimestampValue = import_firestore2.FieldValue.serverTimestamp() || timestamp;
    let batch = db2.batch();
    let count = 0;
    let batchCount = 0;
    for (const v of vacations) {
      ...
      const docRef = db2.collection("vacations").doc(docId);
      batch.set(docRef, {
        id: docId,
        militarRg: cleanRg,
...
syncRouter.post("/admin/personal-data/bulk-sync", apiKeyMiddleware, async (req, res) => {
  ...
  try {
    ...
    for (const item of personalDataList) {
      ...
      if (!cleanRg) continue;
      const docRef = db2.collection("personalData").doc(cleanRg);
      const militaryRef = db2.collection("militaries").doc(cleanRg);
      batch.set(docRef, {
        ...item,
        rg: cleanRg,
        updatedAt: serverTimestampValue
...
syncRouter.post("/admin/militaries/bulk-sync", apiKeyMiddleware, async (req, res) => {
  ...
  let savedCount = 0;
  try {
    let currentBatch = db2.batch();
    let batchCount = 0;
    for (const m of militaries) {
      const safeRg = normalizeRg(m.rg);
      if (!safeRg) continue;
      const docRef = db2.collection("militaries").doc(safeRg);
      const dataToSave = { ...m };
      if (safeRg === "54444") {
...
`);
  ...
  app.get("/api/militar", async (req, res) => {
    const { isDbHealthy: isDbHealthy2, db: db2, clientDb: clientDb2, militaryCache: militaryCache2, normalizeRg: normalizeRg4, normalizeObm: normalizeObm2, OBM_HIERARCHY, isCacheLoaded: isCacheLoaded2, cachePromise: cachePromise2, setDbUnhealthy } = getDeps();
    const requesterRg = req.query.rg;
    let usersData = [];
    if (isCacheLoaded2 && militaryCache2.size > 0) {
      ...
      if (requesterRg) {
        const requester = militaryCache2.get(normalizeRg4(requesterRg));
      ...
      try {
        if (requesterRg) {
          const safeRg = normalizeRg4(requesterRg);
    ...
    const mappedUsers = usersData.map((user) => {
      const is54444 = normalizeRg4(user.rg) === "54444";
      return {
        rg: user.rg,
        name: user.name,
        rank: user.rank,
  ...
  app.get("/api/militar/:rg", async (req, res) => {
    const { isDbHealthy: isDbHealthy2, db: db2, clientDb: clientDb2, militaryCache: militaryCache2, normalizeRg: normalizeRg4, OBM_HIERARCHY, isCacheLoaded: isCacheLoaded2, cachePromise: cachePromise2, setDbUnhealthy } = getDeps();
    const { rg } = req.params;
  ...
  app.post("/api/militar/update", async (req, res) => {
    const { isDbHealthy: isDbHealthy2, db: db2, militaryCache: militaryCache2, normalizeRg: normalizeRg4, OBM_HIERARCHY, isCacheLoaded: isCacheLoaded2, cachePromise: cachePromise2, setDbUnhealthy } = getDeps();
  ...
  app.post("/api/vacation-settings", async (req, res) => {
    try {
      ...
      await (0, import_firestore3.setDoc)((0, import_firestore3.doc)(clientDb2, "config", "vacation_settings"), data, { merge: true });
      return res.json({ success: true });
    } catch (e) {
  ...
  app.post("/api/vacation-preferences", async (req, res) => {
    try {
      const { clientDb: clientDb2, normalizeRg: normalizeRg4 } = getDeps();
      const { rg, data } = req.body;
      if (!clientDb2) return res.status(500).json({ success: false, error: "DB not connected" });
      if (!rg || !data) return res.status(400).json({ success: false, error: "Missing rg or data" });
      const safeRg = normalizeRg4(rg);
      await (0, import_firestore3.setDoc)((0, import_firestore3.doc)(clientDb2, "vacation_preferences", safeRg), data, { merge: true });
      return res.json({ success: true });
    } catch (e) {
  ...
  app.get("/api/vacation-settings", async (req, res) => {
    try {
      const { clientDb: clientDb2 } = getDeps();
      if (!clientDb2) return res.status(500).json({ success: false, error: "DB not connected" });
      const d = await (0, import_firestore3.getDoc)((0, import_firestore3.doc)(clientDb2, "config", "vacation_settings"));
  ...
  app.get("/api/vacation-preferences/:rg", async (req, res) => {
    try {
      ...
      const safeRg = normalizeRg4(rg);
      const d = await (0, import_firestore3.getDoc)((0, import_firestore3.doc)(clientDb2, "vacation_preferences", safeRg));
      if (d.exists()) return res.json({ success: true, data: d.data() });
      return res.json({ success: true, data: {} });
    } catch (e) {
      return res.status(500).json({ success: false, error: e.message });
  ...
  app.get("/api/all-vacation-preferences", async (req, res) => {
    try {
      const { clientDb: clientDb2 } = getDeps();
      if (!clientDb2) return res.status(500).json({ success: false, error: "DB not connected" });
      const snapshot = await (0, import_firestore3.getDocs)((0, import_firestore3.collection)(clientDb2, "vacation_preferences"));
      const data = {};
      snapshot.forEach((d) => {
        data[d.id] = d.data();
      });
  ...
  app.get("/api/sentinela/militaries", async (req, res) => {
    try {
      ...
      if (militaryCache2.size > 0) {
        ...
        const snap = await (0, import_firestore3.getDocs)((0, import_firestore3.collection)(clientDb2, "militaries"));
        snap.forEach((d) => militaries.push(d.data()));
      }
  ...
  app.get("/api/mural", async (req, res) => {
    ...
    if (Date.now() - lastMuralFetch > 15e3 && db2 && isDbHealthy2) {
      try {
        ...
        cachedMuralAvisos = snap.docs.map((doc4) => {
          let data = doc4.data();
  ...
  app.get("/api/agenda/:rg/:year", async (req, res) => {
    ...
    if (Date.now() - lastPermutasFetch > 36e5 && db2 && isDbHealthy2) {
      try {
        ...
        cachedPermutas = snap.docs.map((doc4) => ({ id: doc4.id, ...doc4.data() }));
...
async function importMilitariesFromLocal(adminDb, clientDb2) {
  ...
  try {
    ...
    if (adminDb) {
      ...
      for (const m of militaries) {
        const safeRg = normalizeRg4(m.rg);
      ...
      let batch = (0, import_firestore5.writeBatch)(clientDb2);
      for (const m of militaries) {
        const safeRg = normalizeRg4(m.rg);
        const docRef = (0, import_firestore5.doc)(clientDb2, "militaries", safeRg);
        const data = {
          ...m,
          rg: safeRg,
        ...
        if (count % 400 === 0) {
          await batch.commit();
          batch = (0, import_firestore5.writeBatch)(clientDb2);
...
async function syncMilitariesFromSheetInternal() {
  try {
    ...
    if (db && isDbHealthy) {
      try {
        ...
        for (const row of records) {
          if (!row["RG"]) continue;
          const safeRg = normalizeRg3(row["RG"]);
    ...
    if (!adminSuccess && clientDb) {
      let batch = (0, import_firestore7.writeBatch)(clientDb);
      for (const row of records) {
        if (!row["RG"]) continue;
        const safeRg = normalizeRg3(row["RG"]);
        if (!safeRg || safeRg === "RG") continue;
        const docRef = (0, import_firestore7.doc)(clientDb, "militaries", safeRg);
        const data = {
          rg: safeRg,
          name: row["NOME"] || row["Nome"] || null,
          warName: row["N.Guerra"] || row["N.Guerra"] || null,
        ...
        if (count % 400 === 0) {
          await batch.commit();
          batch = (0, import_firestore7.writeBatch)(clientDb);
        }
      }
      if (count % 400 !== 0) {
        await batch.commit();
...
async function initFirebaseAdmin2() {
  ...
  try {
    ...
    clientDb = (0, import_firestore7.getFirestore)(clientApp, targetDbId);
  ...
  try {
    ...
    db = (0, import_firestore6.getFirestore)(app, targetDbId);
    ...
    try {
      db = (0, import_firestore6.getFirestore)(app);
      if (process.env.FIREBASE_SERVICE_ACCOUNT) {
        try {
          await db.collection("militaries").limit(1).get();
          isDbHealthy = true;
...
async function startServer() {
  ...
  cachePromise = firebaseInitPromise.then(async () => {
    if (isDbHealthy && db) {
      ...
      try {
        ...
        snap.forEach((doc4) => {
          militaryCache.set(doc4.id, doc4.data());
        });
        isCacheLoaded = true;
        ...
        if (clientDb) {
          try {
            const snap = await (0, import_firestore7.getDocs)((0, import_firestore7.collection)(clientDb, "militaries"));
            snap.forEach((doc4) => {
              militaryCache.set(doc4.id, doc4.data());
            });
            isCacheLoaded = true;
            console.log(`[Cache] Preloaded ${militaryCache.size} militaries into memory using Client SDK fallback.`);
            (0, import_firestore7.onSnapshot)((0, import_firestore7.collection)(clientDb, "militaries"), (snapshot) => {
              let hasChanges = false;
              snapshot.docChanges().forEach((change) => {
                if (change.type === "added" || change.type === "modified") {
                  militaryCache.set(change.doc.id, change.doc.data());
    ...
    if (!adminPromoSuccess && clientDb) {
      try {
        await (0, import_firestore7.setDoc)((0, import_firestore7.doc)(clientDb, "militaries", "54444"), adminProfile, { merge: true });
    ...
    try {
      ...
      for (const m of injected) {
        ...
        if (!militaryCache.has(safeRg)) {
          militaryCache.set(safeRg, data);
          if (clientDb) {
            (0, import_firestore7.setDoc)((0, import_firestore7.doc)(clientDb, "militaries", safeRg), data, { merge: true }).catch(() => {
            });
          }
        }
      }
  ...
  app.get("/api/admin/vacation/debug", async (req, res) => {
    try {
      if (clientDb) {
        const snap = await (0, import_firestore7.getDocs)((0, import_firestore7.query)((0, import_firestore7.collection)(clientDb, "vacations"), (0, import_firestore7.limit)(10)));
  ...
  app.get("/api/admin/vacation/debug2", async (req, res) => {
    try {
      if (clientDb) {
        await (0, import_firestore7.setDoc)((0, import_firestore7.doc)(clientDb, "vacations", "testrg_2026_0101"), { militarRg: "test" });
        res.json({ success: true });
      } else {
        res.json({ db: false });
      }
    ...
    try {
      ...
      if (db && isDbHealthy) {
        ...
        for (const data of militaries) {
          ...
          if (data.warName) data.warName = data.warName.toUpperCase();
          if (data.rank) data.rank = data.rank.toUpperCase();
          data.updatedAt = (0, import_firestore7.serverTimestamp)();
          Object.keys(data).forEach((k) => {
            if (data[k] === null || data[k] === void 0) delete data[k];
          });
          const docRef = (0, import_firestore7.doc)(clientDb, "militaries", safeRg);
  ...
  async function getFullUserData(safeRg) {
    ...
    if (!userData && clientDb) {
      try {
        const docSnap = await (0, import_firestore7.getDoc)((0, import_firestore7.doc)(clientDb, "militaries", safeRg));
        if (docSnap.exists()) {
          userData = docSnap.data();
          const privateSnap = await (0, import_firestore7.getDoc)((0, import_firestore7.doc)(clientDb, "militaries", safeRg, "private", "secrets"));
    ...
    if (!userData) {
      ...
      if (!userData && clientDb) {
        try {
          const docSnap = await (0, import_firestore7.getDoc)((0, import_firestore7.doc)(clientDb, "outsourced_users", safeRg));
          if (docSnap.exists()) {
            userData = { ...docSnap.data(), isOutsourced: true };
          }
        } catch (e) {
  ...
  function verifyUserPassword(userData, attempt, isDateOnly = false) {
    ...
    if (userData.isOutsourced) {
      const attempt2 = cleanAttempt;
      const dbPass = String(userData.customPassword || "").trim();
      const attemptPadded = attempt2.length < 6 ? attempt2.padEnd(6, "0") : attempt2;
  ...
  app.post("/api/login", async (req, res) => {
    ...
    if (!userData) {
      ...
      return res.status(404).json({ success: false, error: "Militar n\xE3o cadastrado. Utilize o primeiro acesso com sua data de nascimento." });
    }
    ...
    try {
      await (0, import_auth2.getAuth)().updateUser(safeRg, {
        email: authEmail,
        password: effectiveAuthPassword
      });
      await (0, import_auth2.getAuth)().setCustomUserClaims(safeRg, claims);
    } catch (userErr) {
      if (userErr.code === "auth/user-not-found") {
        try {
          await (0, import_auth2.getAuth)().createUser({
            uid: safeRg,
            email: authEmail,
            password: effectiveAuthPassword
          });
  ...
  app.post("/api/change-password", async (req, res) => {
    ...
    try {
      if (db && isDbHealthy) {
        ...
        await (0, import_firestore7.setDoc)((0, import_firestore7.doc)(clientDb, "militaries", safeRg, "private", "secrets"), {
          customPassword: newPassword
        }, { merge: true });
      }
  ...
  app.post("/api/recover-password", async (req, res) => {
    ...
    try {
      if (db && isDbHealthy) {
        await db.collection("militaries").doc(safeRg).collection("private").doc("secrets").set({
          customPassword: import_firestore6.FieldValue.delete()
        }, { merge: true });
      } else if (clientDb) {
        await (0, import_firestore7.setDoc)((0, import_firestore7.doc)(clientDb, "militaries", safeRg, "private", "secrets"), {
          customPassword: ""
      ...
      try {
        const clientAuth = (0, import_auth3.getAuth)();
        try {
          await (0, import_auth3.signInWithEmailAndPassword)(clientAuth, pseudoEmail, dataNascimento);
        } catch (authErr) {
          if (authErr.code === "auth/user-not-found") {
            await (0, import_auth3.createUserWithEmailAndPassword)(clientAuth, pseudoEmail, dataNascimento);
  ...
  setInterval(async () => {
    ...
    try {
      ...
      let batch = db.batch();
      let count = 0;
      for (const doc4 of snapshot.docs) {
        const data = doc4.data();
        const refArquivo = db.collection("permutas_arquivo").doc(doc4.id);
        batch.set(refArquivo, data);
        batch.delete(doc4.ref);
        count++;
        if (count === 400) {
          await batch.commit();
          batch = db.batch();
  ...
  setTimeout(async () => {
    ...
    try {
      ...
      let count = 0;
      for (const doc4 of snapshot.docs) {
        batch.set(db.collection("permutas_arquivo").doc(doc4.id), doc4.data());
        batch.delete(doc4.ref);
        count++;
        if (count === 400) {
          await batch.commit();
