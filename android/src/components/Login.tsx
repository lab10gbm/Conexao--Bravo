...
export function Login({ onLogin }: { onLogin?: (profile: UserProfile) => void }) {
  const [loading, setLoading] = useState(false);
  const [formData, setFormData] = useState({ rg: '', password: '' });
  const [error, setError] = useState<string | null>(null);

  const [isRecovering, setIsRecovering] = useState(false);
  const [recoverData, setRecoverData] = useState({ rg: '', dataNascimento: '' });
        ...
        const authenticate = async () => {
          if (data.token) {
            try {
               ...
               console.error('[Auth] Custom token sign-in failed, falling back:', err);
            }
          }
          ...
          if (data.verifyCode && auth.currentUser) {
            try {
              ...
              if (linkData.success && linkData.profile) {
                ...
                localStorage.setItem('militar_profile', JSON.stringify(linkData.profile));
              }
            } catch (e) {
  ...
  const handleRecover = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        setRecoverSuccess('Senha redefinida com sucesso! Você já pode acessar o sistema utilizando sua Data de Nascimento (DDMMYYYY) como senha.');
        setTimeout(() => setIsRecovering(false), 5000);
      } else {
        setError(data.error || 'Erro ao recuperar senha. Verifique seus dados.');
      }
    } catch (err) {
      console.error(err);
      setError('Erro de conexão com o servidor');
      ...
      >
        <div className="bg-[var(--color-brand-dark)] p-8 lg:p-12 text-white text-center flex flex-col items-center border-b-4 border-[var(--color-brand-red)]">
           <div className="w-16 h-16 bg-white rounded-full flex items-center justify-center p-2 mb-6">
             <div className="w-full h-full border-2 border-[var(--color-brand-dark)] rounded-full flex items-center justify-center font-black text-[var(--color-brand-dark)] text-[10px]">10ºGBM</div>
           </div>
          ...
          {isRecovering ? (
            <form onSubmit={handleRecover} className="space-y-6">
              <div className="text-center mb-8">
                <p className="text-slate-500 font-bold text-xs leading-relaxed uppercase tracking-wider">
                  ...
                  Informe seu RG e Data de Nascimento para restaurar a senha padrão.
                </p>
              </div>
              ...
              {!recoverSuccess && (
                <>
                  <div className="space-y-5">
                    <div>
                      <label className="block text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1.5 ml-1">RG Militar</label>
                      <input
                        type="text"
                        required
                        placeholder="DIGITE SEU RG"
                        value={recoverData.rg}
                        onChange={(e) => setRecoverData(prev => ({ ...prev, rg: e.target.value }))}
                    ...
                    <div>
                      <label className="block text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1.5 ml-1">Data de Nascimento</label>
                      <input
                        type="text"
                        required
                        placeholder="DDMMYYYY"
                        value={recoverData.dataNascimento}
                        onChange={(e) => setRecoverData(prev => ({ ...prev, dataNascimento: e.target.value }))}
                        className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded focus:border-[var(--color-brand-dark)] focus:ring-0 transition-all font-mono text-xs text-slate-700 placeholder:text-slate-300 uppercase"
                      />
                    </div>
                  </div>
              ...
              <div className="mt-6 text-center">
                <button
                  type="button"
                  onClick={() => { setIsRecovering(false); setError(null); setRecoverSuccess(null); }}
            ...
            <form onSubmit={handleLogin} className="space-y-6">
              <div className="text-center mb-8">
                <p className="text-slate-500 font-bold text-xs leading-relaxed uppercase tracking-wider">
                  {loading && !!localStorage.getItem('militar_profile') ? (
                    <span className="text-amber-600 animate-pulse">Sincronizando Sessão Final... Aguarde.</span>
                  ) : (
                    <>
              ...
              <div className="mt-8 pt-8 border-t border-slate-100 flex flex-col items-center gap-2">
                <span className="text-[9px] font-black text-slate-300 uppercase tracking-widest leading-none">Status do Sistema</span>
